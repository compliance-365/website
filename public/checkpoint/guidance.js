...
window.GUIDANCE = {
  ...
  'A.5.16': {
    how: "Use Entra ID as the single identity source for every system that supports it — no local or shadow accounts — with one identity per person, deactivated promptly on departure. Keep guest accounts to a genuine business need and review them periodically, and where Entra ID Governance is licensed, automate joiner/leaver processing with Lifecycle Workflows rather than relying on someone remembering; Checkpoint's guest-count check flags guest drift and its lifecycle-workflows check confirms that automation is actually configured and enabled. Where the automation was never there or did not fire, dormant-accounts is the backstop: it reads per-account sign-in activity and lists ENABLED accounts nobody has used in your review window — the offboarding that was never started, rather than the one that was left half-finished. Break-glass accounts legitimately appear in that list, so record which of yours are deliberate.",
    evidence: "Entra's user and guest list, an offboarding record showing prompt deactivation, the Lifecycle Workflows configuration where licensed, and Checkpoint's guests/lifecycle-workflows/dormant-accounts scan results.",
    link: "https://entra.microsoft.com",
    path: "Identity → Users → All users (filter: User type = Guest)",
    checks: ["guests", "lifecycle-workflows", "dormant-accounts"]
  },
  'A.5.17': {
  ...
  'A.5.18': {
    ...
    evidence: "The Entra PIM configuration, an access-review report, and Checkpoint's PIM/access-review/priv-role-changes/dormant-accounts scan results.",
    link: "https://entra.microsoft.com",
  ...
  'A.8.1': {
    how: "Enrol every endpoint accessing company data into Intune, apply a compliance policy (encryption, OS version, security baseline) and block access from non-compliant devices through Conditional Access. Checkpoint's device-compliance and compliance-policy checks read the Intune side of this directly on every scan; its ca-device check separately confirms Conditional Access is actually enforcing that compliance requirement at sign-in, not just recording it in Intune. Two further checks read the endpoint's own security state rather than the existence of a policy about it: device-encryption (is the disk actually encrypted) and device-jailbroken (is the device still trustworthy enough for its compliance report to mean anything). Both are worth reading alongside the compliance percentage, because a compliance policy only tests what it was configured to test.",
    evidence: "The Intune compliance policy configuration, device compliance reporting, the Conditional Access policy gating cloud app access on device compliance, and Checkpoint's device/compliance-policy/device-encryption/device-jailbroken/ca-device scan results.",
    link: "https://intune.microsoft.com",
    path: "Devices → Compliance → Policies",
    checks: ["device", "compliance-policy", "device-checkin", "device-encryption", "device-jailbroken", "ca-device"]
  },
  'A.8.2': {
  ...
  'A.8.19': {
    how: "Control what software can run on managed devices with application control — Windows Defender Application Control (WDAC) or AppLocker deployed via Intune, restricting installation to an approved list. Checkpoint's wdac check reads whether this is genuinely in place via your Secure Score signal. On mobile, the equivalent question is whether the platform's own controls are still intact at all: device-jailbroken flags enrolled iOS/Android devices reporting as jailbroken or rooted, where application control and every other on-device restriction can be defeated locally.",
    evidence: "The Intune application-control policy deployment, and Checkpoint's wdac and device-jailbroken scan results.",
    link: "https://intune.microsoft.com",
    checks: ["wdac", "device-jailbroken"]
  },
  'A.8.20': {
  ...
  'A.8.24': {
    ...
    link: "https://portal.azure.com",
    checks: ["encryption", "device-encryption", "aws-ebs-encryption", "aws-rds-encryption"]
  },
  'A.8.25': {
