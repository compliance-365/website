...
| `AuditLog.Read.All` | Read | `legacy-auth-observed`, `priv-role-changes`, `dormant-accounts`, `mfa-registration` | Reads the Entra sign-in log and directory audit log — the only two checks here that report what the tenant **did** rather than how it is **configured**. That is why the permission is worth adding to an unattended monitor specifically: a legacy sign-in succeeding overnight bypasses MFA regardless of policy, and no configuration check in this Function can see it. Read-only, and the monitor never writes to or purges an audit log; software that could alter its own evidence would be worthless as an audit input. Sign-in log reads are bounded by a row cap (`gCapped`) rather than following every `nextLink` — a tenant under a credential-stuffing run against IMAP can have hundreds of thousands of rows in the window. If this permission is not granted, these checks report Manual rather than failing. The same scope also backs `dormant-accounts` (per-account `signInActivity`, which additionally needs Entra ID P1 — a free-tier tenant reports Manual) and `mfa-registration` (the authentication methods registration report, which needs no premium tier). Neither is a new consent decision: both spend a permission already granted for the two checks above. |
