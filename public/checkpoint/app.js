...
(function () {
  ...
  /* ── CSV import (the other half of the story) ──────────────────────
     Every register above exports; until now none imported, so an
     organisation arriving with a risk register in a spreadsheet -- or
     migrating off another GRC tool -- hand-keyed it row by row. That is
     the single largest onboarding cost in the product.

     Import is deliberately narrower than export. Only Risks and Actions
     are importable, because those are the two registers an organisation
     actually arrives holding; the rest (audit log, calendar, controls)
     are generated by Checkpoint itself and a bulk write into them would
     be forging a record, not migrating one.

     Three rules keep this safe on a live register:

       ADD ONLY. Import never updates or overwrites an existing record.
       Any ID column in the file is ignored and a fresh sequential ID is
       assigned, exactly as the manual add-forms do. A re-import of the
       same file therefore duplicates rows -- visible and fixable --
       rather than silently rewriting records a practitioner has since
       edited.

       NOTHING WRITES BEFORE A HUMAN SEES THE PLAN. parse -> validate ->
       show exactly what will happen -> confirm -> write.

       A BAD ROW SKIPS ITSELF, NOT THE FILE. One malformed likelihood in
       row 40 must not cost the practitioner the other 39.

     Column validation uses the app's own vocabularies, so an import
     cannot introduce a priority or treatment the UI has no way to
     display. */
  function oneOf(label, allowed) {
    return function (v) {
      var hit = allowed.find(function (a) { return a.toLowerCase() === v.toLowerCase(); });
      return hit ? null : label + ' must be one of: ' + allowed.join(', ');
    };
  }
  function pick(v, allowed, fallback) {
    if (!v) return fallback;
    var hit = allowed.find(function (a) { return a.toLowerCase() === String(v).toLowerCase(); });
    return hit || fallback;
  }
  function scale1to5(label) {
    return function (v) { return /^[1-5]$/.test(v) ? null : label + ' must be a number from 1 to 5'; };
  }
  /* Accepts the ISO dates Checkpoint exports and the d/m/y most
     Australian spreadsheets produce, and rejects anything it cannot
     read rather than storing a date the calendar will silently drop. */
  ...
  /* A file picker with no markup of its own, so adding an import button
     ...
     which, unhelpfully, browsers report only as "no change event ever
     fired", hence the focus-based cleanup rather than a cancel event. */
  function pickTextFile(accept) {
    return new Promise(function (resolve) {
      ...
      var settled = false;
      function done(v) { if (settled) return; settled = true; input.remove(); resolve(v); }
      input.addEventListener('change', function () {
        var file = input.files && input.files[0];
        if (!file) { done(null); return; }
        var reader = new FileReader();
        reader.onload = function () { done(String(reader.result)); };
        reader.onerror = function () { toast('Could not read that file'); done(null); };
        reader.readAsText(file);
      });
      /* Nothing fires on cancel in most browsers; regaining window focus
         without a change event is the only reliable signal. */
      window.addEventListener('focus', function onFocus() {
        window.removeEventListener('focus', onFocus);
        setTimeout(function () { if (!(input.files && input.files.length)) done(null); }, 300);
      });
  ...
  /* Skipped rows are listed individually, with the line number a human
     sees in Excel, because "3 rows were skipped" is useless to somebody
     who has to go and fix them. Capped so a thoroughly broken file
     produces a readable dialog rather than a wall of text. */
  function skippedSummary(plan) {
    var lines = plan.skipped.slice(0, 8).map(function (s) { return 'Line ' + s.line + ' — ' + s.reason; });
    if (plan.skipped.length > 8) lines.push('…and ' + (plan.skipped.length - 8) + ' more');
    return plan.skipped.length + ' row' + (plan.skipped.length === 1 ? '' : 's') + ' will be skipped:\n' + lines.join('\n');
  }

  var IMPORT_REGISTERS = [
    {
      key: 'risks', label: 'Risks',
      columns: [
        { key: 'Risk', aliases: ['Title', 'Risk statement', 'Description'], required: true },
        { key: 'Category', aliases: ['Cat'] },
      ...
      /* Mirrors addManualRisk() exactly, minus the DOM: same ID scheme,
         same defaults, same shape -- an imported risk must be
         indistinguishable from a hand-entered one to every other part
         of the app (residual scoring, closure, reporting). */
      apply: async function (rec, seq, base) {
        var r = {
          id: 'R-' + String(base + 1 + seq).padStart(3, '0'),
          title: rec.Risk,
          cat: rec.Category || 'Uncategorised',
          src: 'CSV import',
    ...
    },
    {
      ...
      columns: [
        { key: 'Title', aliases: ['Action', 'Task', 'Finding'], required: true },
        { key: 'Type', validate: oneOf('Type', ACTION_TYPES) },
        { key: 'Control', aliases: ['Control code'] },
        { key: 'Priority', aliases: ['Pr'], validate: oneOf('Priority', ACTION_PRIORITIES) },
        { key: 'Owner' },
        { key: 'Owner email', aliases: ['OwnerEmail', 'Email'], validate: function (v) { return /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(v) ? null : 'Owner email is not a valid email address'; } },
        { key: 'Due', aliases: ['Due date'], validate: function (v) { return normaliseDate(v) ? null : 'Due must be a date (YYYY-MM-DD or DD/MM/YYYY)'; } }
      ],
      /* Deliberately does NOT accept a Risk column. Linking an action to
         a risk drives residual re-scoring and risk closure, and a link
         to an ID that does not exist in this tenant would leave a risk
         permanently un-closable. Imported actions are linked in the UI,
         where the picker can only offer real risks. */
      maxId: function () { return S.actions.reduce(function (m, a) { var n = parseInt(String(a.id).replace(/\D/g, ''), 10) || 0; return Math.max(m, n); }, 0); },
      apply: async function (rec, seq, base) {
        var a = {
          id: 'ACT-' + String(base + 1 + seq).padStart(3, '0'),
          title: rec.Title,
          type: pick(rec.Type, ACTION_TYPES, 'Action'),
          risk: '', control: rec.Control || '',
  ...
  window.App = {
    ...
    /* The import counterpart. Reads a file the practitioner picks,
       shows exactly what would happen, and only writes once they say
       yes. See the IMPORT_REGISTERS comment above for why this is
       add-only and why only Risks and Actions are importable.

       Writes are sequential and individually caught: SharePoint can
       reject one row (a throttle, a field-length limit) without that
       costing the practitioner the rest of the batch. Whatever
       happened, good and bad, is reported honestly at the end and
       written to the audit log -- an import that half-succeeded must
       never look like one that fully succeeded. */
    importCsv: async function (key) {
      var reg = IMPORT_REGISTERS.find(function (r) { return r.key === key; });
      if (!reg) return;

      var text = await pickTextFile('.csv,text/csv');
      if (text == null) return;

      var plan = window.CheckpointLib.planCsvImport(text, reg);
      if (plan.error) { await showModal({ title: 'Import ' + reg.label, message: plan.error + '\n\nExpected columns: ' + reg.columns.map(function (c) { return c.key + (c.required ? ' (required)' : ''); }).join(', '), confirmText: 'Close', cancelText: 'Cancel' }); return; }
      if (!plan.ready.length) { await showModal({ title: 'Import ' + reg.label, message: 'Nothing in this file can be imported.\n\n' + skippedSummary(plan), confirmText: 'Close', cancelText: 'Cancel' }); return; }

      var msg = plan.ready.length + ' ' + reg.label.toLowerCase().replace(/s$/, '') + (plan.ready.length === 1 ? '' : 's') + ' will be added to the register.\n';
      msg += '\nNothing existing is changed — import only ever adds. Each row gets a new ID; any ID column in the file is ignored.\n';
      if (plan.unknownColumns.length) msg += '\nIgnored column' + (plan.unknownColumns.length === 1 ? '' : 's') + ': ' + plan.unknownColumns.join(', ') + '\n';
      if (plan.skipped.length) msg += '\n' + skippedSummary(plan);

      var go = await showModal({ title: 'Import ' + plan.ready.length + ' row' + (plan.ready.length === 1 ? '' : 's') + ' into ' + reg.label, message: msg, confirmText: 'Import ' + plan.ready.length + ' row' + (plan.ready.length === 1 ? '' : 's') });
      if (!go) return;

      busy(true);
      var added = [], failed = [], base = reg.maxId();
      for (var i = 0; i < plan.ready.length; i++) {
        try { added.push(await reg.apply(plan.ready[i].rec, added.length, base)); }
        catch (e) { failed.push('line ' + plan.ready[i].line + ' — ' + (e && e.message ? e.message : 'write failed')); }
      }
      busy(false);

      var summary = added.length + ' added' + (failed.length ? ', ' + failed.length + ' failed to write' : '') + (plan.skipped.length ? ', ' + plan.skipped.length + ' skipped as invalid' : '');
      audit('Register imported (CSV)', 'Import', reg.key, '(none)', summary + (added.length ? ' — ' + added.join(', ') : ''));
      log('Imported <b>' + esc(reg.label) + '</b> from CSV: ' + esc(summary) + '.');
      if (failed.length) {
        await showModal({ title: 'Import finished with errors', message: summary + '\n\nThese rows could not be written:\n' + failed.slice(0, 10).join('\n') + (failed.length > 10 ? '\n…and ' + (failed.length - 10) + ' more' : ''), confirmText: 'Close', cancelText: 'Cancel' });
      } else toast('<b>' + added.length + '</b> row' + (added.length === 1 ? '' : 's') + ' imported');
      renderAll();
    },
