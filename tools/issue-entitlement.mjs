#!/usr/bin/env node
/* Compliance365 — entitlement file issuance CLI
 * ------------------------------------------------------------
 * Generates the Ed25519 keypair Checkpoint verifies entitlement files
 * against, and issues signed entitlement files for clients: a small
 * JSON document naming which frameworks a tenant is entitled to, an
 * expiry date, and an Ed25519 signature over the two. Checkpoint's own
 * client-side verification lives in public/checkpoint/lib.js
 ...
 * whole scheme depends on: anyone holding it can issue entitlement
 ...
 * or secrets vault, rotate config.js's entitlementPublicKey and
 * re-issue every client's file if it's ever exposed).
 *
 * Usage:
 ...
 *     Issues a signed entitlement file for one client tenant.
 *
 *   node tools/issue-entitlement.mjs verify
 *     --file acme-corp-entitlement.json
...
async function cmdIssue(args) {
  ...
  if (!tenant) fail('--tenant is required (the client\'s Entra tenant ID — Directory ID in the Entra admin center).');
  ...
  if (!outPath) fail('--out is required (path to write the signed entitlement JSON file to).');
  ...
  console.log('Entitlement file written to: ' + outPath);
  console.log('Tenant: ' + tenant);
  console.log('Frameworks: ' + frameworks.join(', '));
  console.log('Expiry: ' + expiry);
  console.log('');
  console.log('Send this file to the client\'s practitioner to upload in Checkpoint\'s Frameworks view.');
}
...
async function cmdVerify(args) {
  ...
  if (!raw.payload || !raw.signature) fail('Not a valid entitlement file — missing payload/signature.');
  ...
  const evalResult = evaluateEntitlement(raw.payload, raw.payload.tenantId, new Date().toISOString().slice(0, 10));
  console.log('Tenant: ' + raw.payload.tenantId);
  console.log('Frameworks: ' + raw.payload.frameworks.join(', '));
  console.log('Issued: ' + raw.payload.issuedAt + '  Expiry: ' + raw.payload.expiry);
  console.log('Status (as of today): ' + evalResult.status);
}
...
async function main() {
  ...
  console.log('  node tools/issue-entitlement.mjs issue --tenant ID --frameworks a,b,c --expiry YYYY-MM-DD --key entitlement-private.json --out FILE.json');
